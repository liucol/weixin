...
var C9377 = {
	https: https,
	url: https ? 'https://www.9377.cn' : 'http://www.9377.cn/',
	app_url: https ? 'https://wvw.9377.cn' : 'http://wvw.9377.cn/',
	special_url: https ? 'https://special.9377.cn' : 'http://special.9377.cn/',
	resource: https ? 'https://static.9377.cn' : 'http://statics.9377s.com/',
	build: ''
};

function setcookie(name, value, expires, path, domain, secure){
	var str = escape(name) +'='+ escape( value ) +
		(expires ? ';expires='+ (new Date( (new Date(expires * 1000)).getTime())).toGMTString() : '' ) + 
		(path ? ';path=' + path : '' ) +
...
C9377.setcookie = function(name, value, expires){
	var domain = 'https://static.9377.cn/js/9377.cn';
...
function preg_match_all(reg, string){
	var modifier = 'g';
	if(reg.ignoreCase) modifier += 'i';
	if(reg.multiline) modifier += 'm';
	reg.compile(reg.source, modifier);
	
	var ret = [];
	string.replace(reg, function(m1){
		if(ret[0] === undefined) ret[0] = [];
		ret[0].push(m1);
		
		var length = arguments.length -2;
		for(var i = 1; i < length; i++){
			if(ret[i] === undefined) ret[i] = [];
			ret[i].push(arguments[i]);
		}
	});
	return ret;
}

C9377.include = function(f, success){
	C9377.include.including[f] = 1;
	...
	script.onload = script.onreadystatechange = function() {
		if ( !done && (!this.readyState ||
			...
			C9377.include.included[f] = 1;
			delete C9377.include.including[f];
...
C9377.include_once = function(f, success){
	if(C9377.include.including[f]){
		setTimeout(function(){ C9377.include_once(f, success); }, 300);
		return false;
	}
	if(C9377.include.included[f]){
		success(f);
		return true;
	}
	
	C9377.include(f, success);
};

C9377.include.included = {};
C9377.include.including = {};
...
function ajax_parameters(o, p) {
    ...
    for (i in o) {
        if (typeof o[i] == 'object' && o[i] !== null && !o[i].splice)
...
$(function($) {
    //print_r($_COOKIE);
    if ($_COOKIE['forum_sync_js']) {
        var src = preg_match_all(/src="([^"]+)"/g, $_COOKIE['forum_sync_js']);
        
        try {
            for (var i = 0; i < src[1].length; i++) {
                include_once(src[1][i], function(file) {
                    C9377.setcookie('forum_sync_js', 0, -1);
                });
            }
        } catch (e) {
        }
    ...
    if ($_COOKIE['admin_url']) {
        function _data_url(_this, action) {
			console.log($_COOKIE['admin_url']); 
            
			var m = $_COOKIE['admin_url'].match(/\?domain=([^&]+)/);
			var domain = document.domain.match(/[^\.]+\.[^\.]+(?:\.[^\.]{2,2})?$/);
			var tmp = _this.attr('data_id').match(/\[(\d+)\]/);
			var id = tmp ? tmp[1] : '';
			var param = ajax_parameters({
				id: id,
				page: _this.attr('page'),
				postfix: _this.attr('postfix'),
				name: _this.find('span').html(),
				domain: m && m[1] ? m[1] : ($_GET['domain'] ? $_GET['domain'] : 'https://static.9377.cn/js/s9377.com'),
				referer: window.location.href
			});
			
			var url = '/data.php?act=add&id='+ id +'&ext='+ encodeURIComponent(param);
			if(m && m[1]){
        ...
        try{
            ...
            .dblclick(function() {
                ...
                $(this).find('div').css({
                    ...
                    width: '16px',
                    height: '16px',
                    background: 'url(/images/se-resize.png) no-repeat -8px -8px',
                    right: '0px',
                    bottom: '0px'
                })
                );
            ...
            if (count) {
                ...
                if (/msie 6/i.test(navigator.userAgent)) {
                    ...
                    $(window).scroll(function() {
                        toggle.css({top: $(window).scrollTop()});
                    });
                }
                
                $('<a id="w3c_v" href="http://validator.w3.org/check?uri=' + encodeURIComponent(window.location.href) + '&group=1" style="cursor: pointer; position: fixed; top: 0px; left: 0px; z-index: 2147483647;" target="_blank" title="w3c validate"><img src="' + C9377.resource + '/images/w3c.png" width="36" height="24" /></a>').appendTo(document.body);
            }
		}catch(e){}
    }
    ...
    $(document).keyup(function(e) {
        if (e.altKey && e.keyCode == 57) {
            if ($_COOKIE['tp_test']) {
                C9377.setcookie('tp_test', 0, -1);
            } else {
                C9377.setcookie('tp_test', 1);
            }
